...
Когда вы впервые запускаете бота командой `/start`, он попросит вас создать **мастер-пароль**.

![Скриншот запроса на создание пароля](https://i.ibb.co/68qGzH0/step1-create-password.png)

Это самый важный шаг для обеспечения вашей приватности. Этот пароль — единственный ключ к вашим данным.

> ❗️ **Критически важно:** Если вы забудете этот пароль, восстановить доступ к данным будет **невозможно**. У нас нет функции "сбросить пароль", так как мы не храним ваш пароль в открытом виде. Это гарантия вашей приватности. Запишите пароль и храните его в надежном месте.
...
![Скриншот анкеты](https://i.ibb.co/NdbvMTWJ/Screenshot-34.png)
...
> **Как это работает?** Если вы введете пароль паники вместо основного, бот сделает вид, что сессия успешно разблокирована, но на самом деле немедленно и безвозвратно удалит всю вашу историю и память.

Вы можете настроить его сразу или пропустить этот шаг и вернуться к нему позже.
...
2.  Нажмите на кнопку **"Create API key in new project"**.
3.  Скопируйте сгенерированный ключ.

    ![Инструкция по созданию ключа](https://i.ibb.co/hZ0gW7p/api-key-creation.gif)
