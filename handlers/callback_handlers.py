...
from config.settings import (
    ...
    CALLBACK_PROFILE_EDIT_PREFIX, STATE_PROFILE_EDITING_ANSWER,
    CALLBACK_PROFILE_SHOW_EDIT_OPTIONS, CALLBACK_PROFILE_BACK_TO_VIEW,
...
# =============================================================================
# --- ОБЪЯВЛЕНИЕ ВСЕХ ФУНКЦИЙ-ОБРАБОТЧИКОВ ---
# Они расположены здесь в алфавитном порядке для удобства навигации.
# Главный маршрутизатор handle_callback_query находится в самом низу.
# =============================================================================
...
async def handle_back_to_profile_view(bot: AsyncTeleBot, call: types.CallbackQuery, lang_code: str):
    """Возвращает профиль в режим просмотра."""
    user_id = call.from_user.id
    ...
    await tg_helpers.edit_message_text_safe(bot, call.message.chat.id, call.message.message_id, full_text, reply_markup=keyboard)
    await tg_helpers.answer_callback_query(bot, call)
...
async def handle_profile_edit_start(bot: AsyncTeleBot, call: types.CallbackQuery, lang_code: str):
    """Начинает процесс редактирования одного поля профиля, задавая правильный вопрос."""
    user_id = call.from_user.id
    question_key = call.data[len(settings.CALLBACK_PROFILE_EDIT_PREFIX):]

    await bot.set_state(user_id, settings.STATE_PROFILE_EDITING_ANSWER, call.message.chat.id)
    await bot.add_data(user_id, call.message.chat.id, question_to_edit=question_key)
    
    await tg_helpers.delete_message_safe(bot, call.message.chat.id, call.message.message_id)

    # --- ИСПРАВЛЕНИЕ: Формируем правильный "редакторский" вопрос ---
    field_name = loc.get_text(f'profile_label_{question_key}', lang_code)
    question_text = loc.get_text('profile_edit_prompt', lang_code).format(field_name=field_name)
    
    # Проверяем, есть ли для этого вопроса кнопки (например, для стиля)
    markup = None
    if QUESTIONNAIRE[question_key]['type'] == 'choice':
        # Если есть, то нужно показать клавиатуру выбора, а не просить текст
        from utils.markup_helpers import _create_choice_keyboard # Локальный импорт
...
async def handle_show_profile_edit_options(bot: AsyncTeleBot, call: types.CallbackQuery, lang_code: str):
    """Переключает профиль в режим редактирования."""
    user_id = call.from_user.id
    fernet_instance = tg_helpers.user_session_keys.get(user_id)
    ...
    full_text = (
        f"{loc.get_text('profile_combined_title', lang_code)}\n\n"
        f"{info_text}\n\n"
        f"_{loc.get_text('profile_edit_mode_desc', lang_code)}_"
    )
    
    keyboard = mk.create_profile_edit_menu_keyboard(lang_code)
    await tg_helpers.edit_message_text_safe(bot, call.message.chat.id, call.message.message_id, full_text, reply_markup=keyboard)
    await tg_helpers.answer_callback_query(bot, call)
...
# =============================================================================
# --- ГЛАВНЫЙ МАРШРУТИЗАТОР И РЕГИСТРАЦИЯ ---
# =============================================================================
...
async def handle_callback_query(call: types.CallbackQuery, bot: AsyncTeleBot):
    ...
    try:
        ...
        elif data == CALLBACK_PROFILE_SHOW_EDIT_OPTIONS:
            await handle_show_profile_edit_options(bot, call, lang_code)
        elif data == CALLBACK_PROFILE_BACK_TO_VIEW:
            await handle_back_to_profile_view(bot, call, lang_code)
        elif data.startswith(settings.CALLBACK_PROFILE_EDIT_PREFIX):
            await handle_profile_edit_start(bot, call, lang_code)
